...
3) Always read the README file first.
   
---
## Human and AI Guardrails

1) In every AI instruction block (or prompt), add "Before generating or modifying any Schema, read the existing Schema files and match its declared $schema dialect exactly. Do not upgrade drafts unless explicitly instructed."
...
See: docs/GUT-Check Product Specification.pdf

In the event of conflict between this README and the Product Specification document, the specification document takes precedence and the conflict should be documented.
